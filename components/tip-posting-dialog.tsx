...
import { createTip, type CreateTipData } from "@/lib/actions/tips"
import { useToast } from "@/hooks/use-toast"

interface TipPostingDialogProps {
  onSuccess?: () => void
}
...
export default function TipPostingDialog({ onSuccess }: TipPostingDialogProps) {
  ...
  const handleInputChange = (field: keyof CreateTipData, value: any) => {
    setFormData((prev) => ({
      ...prev,
      [field]: value,
    }))
  ...
  const handleSubmit = async (isDraft = false) => {
    ...
    try {
      ...
      if (result.success) {
        toast({
          title: "Success",
          description: `Tip ${isDraft ? "saved as draft" : "published"} successfully!`,
        })
        ...
        toast({
          title: "Error",
          description: result.error || "Failed to save tip",
          variant: "destructive",
        })
      }
    } catch (error) {
    ...
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>Share your knowledge and help others succeed in their careers</DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6">
          ...
          <div className="flex justify-end gap-3">
            ...
            >
              ...
              {isSubmitting ? "Publishing..." : "Publish Tip"}
            </Button>
          </div>
        </div>
      </DialogContent>
